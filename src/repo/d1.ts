...
function toLegacyGitHubAccount(
  row: {
    user_id: number;
    token: string;
    login: string;
  ...
  // Legacy D1 deployments may still be missing github_accounts.account_type;
  // default to the original single-account behavior until the migration runs.
  return {
    token: row.token,
